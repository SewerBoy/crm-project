...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#searchClueBtn").click(function() {
			queryClueByCondition(1 , 10, 1);
		});

	});

	//封装查询
	function queryClueByCondition(pageNo, pageSize, flag) {
			...
			success:function(data) {
				//把查到的数据显示出来
					...
					$.each(data.clues, function(index, obj) {
						...
						//htmlStr += "<td><a style='text-decoration: none; cursor: pointer;' onclick='window.location.href=\"workbench/clue/detailClue.do?id=\"+obj.id+\"\"'>"+obj.fullname+"</a></td>";
						// htmlStr += "<td><a style=\"text-decoration: none; cursor: pointer;\" onclick=\"window.location.href='workbench/clue/detailClue.do?id='"+obj.id+";\">"+obj.fullname+"</a></td>";
						htmlStr += "<td><a style="+"text-decoration: none; cursor: pointer;"+"onclick="+"window.location.href="+"workbench/clue/detailClue.do?id="+obj.id+"+">"+</a></td>";
					...
					//计算总页数
				var totalPages=1;
				var number = data.count/pageSize;
				if (data.count%pageSize == 0) {
					...
					$("#demo_page").bs_pagination({
						...
						onChangePage:function(event, pageObj) {
							//event:切换事件本身,pageObj:本页面对象
							queryClueByCondition(pageObj.currentPage, pageObj.rowsPerPage, 0);
						}
					});
